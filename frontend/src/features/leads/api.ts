...
export async function getLeadsApi(organizationId: string): Promise<Lead[]> {
    const response = await apiClient(`/organizations/${organizationId}/leads`, { method: "GET" });
    const data = await response.json();
    return data?.data as Lead[] || [];
}

export async function createLeadApi(lead: CreateLeadRequestData): Promise<Lead> {
